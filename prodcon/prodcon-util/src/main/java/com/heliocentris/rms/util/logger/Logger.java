 ...
 */
public class Logger extends LoggerWrapper //implements fdx.logger.Logger
{
    ...
    static String[] ENTERING_MESSAGES =
    {
            "ENTER ", "ENTER {}", "ENTER {}, {}", "ENTER {}, {}, {}"
    };
    static String[] EXITING_MESSAGES =
    {
    ...
    public void info(String format, Object... args)
    {
        if (instanceofLAL && logger.isInfoEnabled(INFO_MARKER))
        {
            ...
            ((LocationAwareLogger) logger).log(INFO_MARKER, FQCN, LocationAwareLogger.INFO_INT, tuple.getMessage(), null, null);
        }
    }
    ...
    public void config(String message)
    {
        if (instanceofLAL)
        {
            ...
            ((LocationAwareLogger) logger).log(CONFIG_MARKER, FQCN, LocationAwareLogger.WARN_INT, tuple.getMessage(), null, null);
        }
    }
    ...
    public void severe(String message)
    {
        if (instanceofLAL)
        {
            ...
            ((LocationAwareLogger) logger).log(SEVERE_MARKER, FQCN, LocationAwareLogger.ERROR_INT, tuple.getMessage(), null, null);
        }
    }
    ...
    public void entering()
    {
        ...
        {
            ((LocationAwareLogger) logger).log(ENTERING_MARKER, FQCN, LocationAwareLogger.DEBUG_INT, ENTERING_MESSAGES[0], null, null);
        }
    }
    ...
    public void entering(Object arg)
    {
        if (instanceofLAL && logger.isTraceEnabled(ENTERING_MARKER))
        {
            //String formattedMessage = MessageFormatter.format(ENTERING_MESSAGES[1], arg);
            FormattingTuple tuple = MessageFormatter.format(ENTERING_MESSAGES[1], arg);
            ((LocationAwareLogger) logger).log(ENTERING_MARKER, FQCN, LocationAwareLogger.DEBUG_INT, tuple.getMessage(), null, null);
        }
    }
    ...
    public void catching(Throwable throwable)
    {
        if (instanceofLAL)
        {
            ((LocationAwareLogger) logger).log(THROWING_MARKER, FQCN, LocationAwareLogger.ERROR_INT, "CATCHING ", null, throwable);
        }
    }
    ...
    public void exiting()
    {
        ...
        {
            ((LocationAwareLogger) logger).log(EXITING_MARKER, FQCN, LocationAwareLogger.DEBUG_INT, EXITING_MESSAGES[0], null, null);
        }
    }
    ...
    public <T> T exiting(T result)
    {
        if (instanceofLAL && logger.isTraceEnabled(EXITING_MARKER))
        {
            //String formattedMessage = MessageFormatter.format(EXITING_MESSAGES[1], result);
            FormattingTuple tuple = MessageFormatter.format(EXITING_MESSAGES[1], result);
            ((LocationAwareLogger) logger).log(EXITING_MARKER, FQCN, LocationAwareLogger.DEBUG_INT, tuple.getMessage(), null, null);
        }
        return result;
    }
    ...
    public <T> T[] exiting(T... args)
    {
        if (instanceofLAL && logger.isTraceEnabled(EXITING_MARKER))
        {
            ...
            //String formattedMessage = MessageFormatter.arrayFormat(messagePattern, args);
            FormattingTuple tuple = MessageFormatter.arrayFormat(messagePattern, args);
            ((LocationAwareLogger) logger).log(EXITING_MARKER, FQCN, LocationAwareLogger.DEBUG_INT, tuple.getMessage(), null, null);
        }
        return args;
